...
export function CameraCapture({ onCapture, onClose }: CameraCaptureProps) {
  ...
  const startCamera = React.useCallback(async (deviceId: string | null) => {
    // Defers state updates to the next tick to prevent synchronous setState inside useEffect warning
    setTimeout(() => {
      setLoading(true)
      setError(null)
    }, 0)
  ...
  const handleSwitchCamera = () => {
    ...
    // Stop tracks of current stream before opening new one
    if (streamRef.current) {
      streamRef.current.getTracks().forEach((track) => track.stop())
      streamRef.current = null
    }
  ...
  return (
    <div className="relative flex flex-col items-center justify-center bg-zinc-950 text-white border border-zinc-800 rounded-2xl overflow-hidden min-h-[300px] sm:min-h-[400px] aspect-square sm:aspect-video">
      ...
      {/* Canvas for rendering capture (hidden) */}
      <canvas ref={canvasRef} className="hidden" />

      {/* Target box / Scan Box overlay (premium design) */}
      {!loading && !error && (
      ...
      {error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-950 text-center p-6 z-20">
          ...
          <div className="flex gap-3">
            {onClose && (
              <Button variant="outline" onClick={onClose} className="rounded-xl border-zinc-800 text-zinc-300 hover:bg-zinc-900">
                Kembali
              </Button>
            )}
            <Button onClick={() => startCamera(activeDeviceId)} className="rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-semibold">
              Coba Lagi
            </Button>
          </div>
        </div>
      ...
      {!loading && !error && (
        <div className="absolute bottom-4 left-0 right-0 flex items-center justify-between px-6 z-20 pointer-events-auto">
          {/* Secondary Left Action (e.g. Back/Close if needed) */}
